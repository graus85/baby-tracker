...
import { getById, insertHeight, updateHeight } from '../../lib/api'
...
export default function HeightForm(){
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try{
        const row = await getById('heights', id)
        setDate(row.date)
  ...
  async function onSubmit(e: React.FormEvent){
    ...
    try{
      const payload = { date, time: toPgTimeWithTZ(time+':00'), cm: Number(cm), note: note || null }
      if (isEdit && id) await updateHeight(id, payload)
      else await insertHeight(payload)
  ...
  return (
    <div className="content">
      <form className="card" onSubmit={onSubmit} style={{maxWidth:520, margin:'16px auto', display:'grid', gap:12}}>
        <h2>{isEdit ? t('actions.edit') : t('add.height')}</h2>
        ...
        <label>{t('fields.cm')}<input className="input" inputMode="decimal" value={cm} onChange={e=>setCm(e.target.value)} /></label>
