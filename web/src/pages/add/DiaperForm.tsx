...
import { getById, insertDiaper, updateDiaper } from '../../lib/api'
...
export default function DiaperForm(){
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try{
        const row = await getById('diapers', id)
        setDate(row.date)
        setTime(String(row.time).slice(0,5))
        setPee(Boolean(row.pee))
        setPoop(Boolean(row.poop))
        setNote(row.note || '')
  ...
  async function onSubmit(e: React.FormEvent){
    ...
    try{
      const payload = { date, time: toPgTimeWithTZ(time+':00'), pee, poop, note: note || null }
      if (isEdit && id) await updateDiaper(id, payload)
      else await insertDiaper(payload)
  ...
  return (
    <div className="content">
      <form className="card" onSubmit={onSubmit} style={{maxWidth:520, margin:'16px auto', display:'grid', gap:12}}>
        <h2>{isEdit ? t('actions.edit') : t('add.diaper')}</h2>
