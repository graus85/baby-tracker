...
import { getById, insertOther, updateOther } from '../../lib/api'
...
export default function OtherForm(){
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try{
        const row = await getById('others', id)
        setDate(row.date)
  ...
  async function onSubmit(e: React.FormEvent){
    ...
    try{
      const payload = { date, time: toPgTimeWithTZ(time+':00'), note }
      if (isEdit && id) await updateOther(id, payload)
      else await insertOther(payload)
  ...
  return (
    <div className="content">
      <form className="card" onSubmit={onSubmit} style={{maxWidth:520, margin:'16px auto', display:'grid', gap:12}}>
        <h2>{isEdit ? t('actions.edit') : t('add.other')}</h2>
